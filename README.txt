...
PC#1<--GROUNDSTATION MESSENGER-->GROUNDSTATION/CUBESAT SIMULATOR UHD-->SDR TX RF PORT-->OVER THE AIR/RF CABLES-->SDR RX RF PORT-->GROUNDSTATION/CUBESAT SIMULATOR UHD-->CUBESAT ECHO SERVER-->PC#1
PC#1<--GROUNDSTATION MESSENGER<--GROUNDSTATION/CUBESAT SIMULATOR UHD<--SDR RX RF PORT-->OVER THE AIR/RF CABLES-->SDR TX RF PORT<--GROUNDSTATION/CUBESAT SIMULATOR UHD<--CUBESAT ECHO SERVER-->PC#1

When using GROUNDSTATION/CUBESAT SIMULATOR NO RADIO
PC#1-->GROUNDSTATION MESSENGER-->GROUNDSTATION/CUBESAT SIMULATOR NO RADIO-->CHANNEL SIMULATION BLOCK-->GROUNDSTATION/CUBESAT SIMULATOR NO RADIO-->CUBESAT ECHO SERVER-->PC#1
PC#1-->GROUNDSTATION MESSENGER<--GROUNDSTATION/CUBESAT SIMULATOR NO RADIO<--CHANNEL SIMULATION BLOCK<--GROUNDSTATION/CUBESAT SIMULATOR NO RADIO<--CUBESAT ECHO SERVER-->PC#1

When using GROUNDSTATION UHD and CUBESAT UHD with 2 separate computers
PC#1<--GROUNDSTATION MESSENGER-->GROUNDSTATION UHD-->SDR TX RF PORT-->OVER THE AIR/RF CABLES-->SDR RX RF PORT-->CUBESAT UHD RX PORT-->CUBESAT ECHO SERVER-->PC#2
PC#1<--GROUNDSTATION MESSENGER<--GROUNDSTATION UHD<--SDR RX RF PORT-->OVER THE AIR/RF CABLES-->SDR TX RF PORT<--CUBESAT UHD TX PORT<--CUBESAT ECHO SERVER-->PC#2
